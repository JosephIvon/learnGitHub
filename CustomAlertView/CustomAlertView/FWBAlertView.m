...
{
    static FWBAlertView * shareCenter = nil;
    @synchronized(self) {
        if (!shareCenter)
        {
            shareCenter = [[FWBAlertView alloc] init];
        }
    }
    return shareCenter;
}
...
- (instancetype)initWithTitle:(NSString*)title message:(NSString*)message delegate:(id)del cancelButtonTitle:(NSString*)cancelBtnTitle otherButtonTitles:(NSString*)otherBtnTitles, ...
{
    self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT + 20)];
    if (self)
    {
...
- (void) layoutSubviews
{
    ...
    if (!_isShow)
        [FWBAlertView exChangeOut:_backView dur:0.5];
    if (self.delegate)
    {
